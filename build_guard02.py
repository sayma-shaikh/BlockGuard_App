...
# NEW: Import necessary libraries for volume control on Windows
...
# NEW: Minimum volume level constant
MIN_VOLUME_LEVEL = 0.5  # 50%
...
class BlockGuardApp(ctk.CTk):
    def __init__(self, *args, **kwargs):
        ...
        # NEW: Add threads for volume monitoring
        self.volume_monitor_thread = None
        self.volume_monitor_stop_event = threading.Event()
    ...
    def create_dashboard_frame(self):
        ...
        # NEW: Changed grid to 4 columns to accommodate the new card
        ...
        # NEW: Volume Lock Card
        volume_lock_card = ctk.CTkFrame(dashboard_frame, fg_color=AppColors.CARD, corner_radius=12)
        volume_lock_card.grid(row=0, column=1, padx=10, sticky="nsew")
        volume_lock_card.pack_propagate(False); volume_lock_card.configure(height=120)
        ctk.CTkLabel(volume_lock_card, text="Volume Lock", font=ctk.CTkFont(size=16, weight="bold"), text_color=AppColors.TEXT_PRIMARY).pack(pady=(15, 5))
        self.volume_lock_switch = ctk.CTkSwitch(volume_lock_card, text="", command=self.toggle_volume_lock, progress_color=AppColors.BUTTON)
        self.volume_lock_switch.pack(pady=5)
        self.volume_lock_status_label = ctk.CTkLabel(volume_lock_card, text="", text_color=AppColors.TEXT_SECONDARY)
        self.volume_lock_status_label.pack(pady=(0, 10))
        # NEW: Disable the switch if not on Windows
        if sys.platform != "win32":
            self.volume_lock_switch.configure(state="disabled")
            self.volume_lock_status_label.configure(text="Windows only")


        # Blocked Items Card
        blocked_items_card = ctk.CTkFrame(dashboard_frame, fg_color=AppColors.CARD, corner_radius=12)
        # NEW: Changed column index
        blocked_items_card.grid(row=0, column=2, padx=10, sticky="nsew")
        ...
        # NEW: Changed column index
        protection_card.grid(row=0, column=3, padx=(10, 0), sticky="nsew")
    ...
    def update_dashboard(self):
        ...
        # NEW: Update the volume lock UI
        if self.config.get('volume_lock_enabled', False):
            self.volume_lock_switch.select()
            self.volume_lock_status_label.configure(text="Currently: Enabled", text_color=AppColors.BLUE_ACCENT)
        else:
            self.volume_lock_switch.deselect()
            if sys.platform == "win32": # Only show disabled text if on windows
                self.volume_lock_status_label.configure(text="Currently: Disabled", text_color=AppColors.TEXT_SECONDARY)
    ...
    # NEW: Handler for the volume lock switch
    def toggle_volume_lock(self):
        self.config['volume_lock_enabled'] = bool(self.volume_lock_switch.get())
        self.save_config()
        self.update_dashboard()
        self.apply_all_blocks()
    ...
    # NEW: Method to start the volume monitor thread
    def start_volume_monitor(self):
        if self.volume_monitor_thread and self.volume_monitor_thread.is_alive():
            self.volume_monitor_stop_event.set()
            self.volume_monitor_thread.join()
        
        # Only start if the feature is enabled
        if self.config.get('volume_lock_enabled', False) and sys.platform == "win32":
            self.volume_monitor_stop_event.clear()
            self.volume_monitor_thread = threading.Thread(target=self.monitor_volume_worker, daemon=True)
            self.volume_monitor_thread.start()

    # NEW: Worker thread for monitoring and adjusting volume
    def monitor_volume_worker(self):
        print("Starting volume monitor thread...")
        CoInitialize() # Initialize COM for this thread
        try:
            sessions = AudioUtilities.GetSpeakers()
            interface = sessions.Activate(IAudioEndpointVolume._iid_, 7, None)
            volume = interface.QueryInterface(IAudioEndpointVolume)
            
            while not self.volume_monitor_stop_event.is_set():
                # Unmute if muted
                if volume.GetMute():
                    volume.SetMute(0, None)
                    print("System unmuted by BlockGuard.")
                
                # Enforce minimum volume
                current_volume = volume.GetMasterVolumeLevelScalar()
                if current_volume < MIN_VOLUME_LEVEL:
                    volume.SetMasterVolumeLevelScalar(MIN_VOLUME_LEVEL, None)
                    print(f"Volume adjusted to {MIN_VOLUME_LEVEL*100}% by BlockGuard.")
                
                time.sleep(0.5) # Check every half a second
        except Exception as e:
            print(f"Error in volume monitor: {e}")
        finally:
            CoUninitialize() # Uninitialize COM
            print("Volume monitor thread stopped.")
        ...
        if os.path.exists(CONFIG_FILE):
            try:
                with open(CONFIG_FILE, 'r') as f:
                    # NEW: Ensure new config option has a default
                    config = json.load(f)
                    config.setdefault('volume_lock_enabled', False)
                    return config
            except json.JSONDecodeError:
                pass # If file is corrupted, return default
        # NEW: Default value for the new feature
        return {"blocked_items": [], "incognito_blocked": False, "volume_lock_enabled": False}
